...
- (instancetype) initWithFourTitles:(NSArray *)titles {
    ...
    if(self) {
        ...
        // Make the 4 labels
        for (NSString *curretTitle in self.currentTitles) {
            UILabel *label = [[UILabel alloc] init];
            label.userInteractionEnabled = NO;
            label.alpha = 0.25;
            
            NSUInteger currentTitleIndex = [self.currentTitles indexOfObject:curretTitle];
...
- (void) layoutSubviews {
    ...
    for (UILabel *thisLabel in self.labels) {
        ...
        if (currentLabelIndex % 2 == 0) {
            // if currentLabelIndex is evenly divisible by 2, it's 0 or 2, so on the left
            labelX = 0;
        } else {
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (self.currentLabel != label) {
        // the label being touched is no longer the intial label
